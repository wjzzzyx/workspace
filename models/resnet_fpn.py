...
class ResNet_fpn(nn.Module):
    ...
    def __init__(self, backbone, in_channels, max_channels, out_channels):
        ...
        self.absorb2 = nn.Conv2d(256, 256, 3, padding=1)
        self.shortcut3 = nn.Conv2d(max_channels // 4, 256, 1)
        self.absorb3 = nn.Conv2d(256, 256, 3, padding=1)
        self.shortcut4 = nn.Conv2d(max_channels // 8, 256, 1)
        self.absorb4 = nn.Conv2d(256, 256, 3, padding=1)
        ...
        self.output = nn.Sequential(
            torch.nn.Conv2d(192, 128, 3, padding=1),
            torch.nn.ReLU(inplace=True),
            torch.nn.Upsample(scale_factor=2, mode='bilinear'),
    ...
    def forward(self, x):
        self.c1 = self.relu(self.bn1(self.conv1(x)))
        ...
        h = torch.cat([self.c1, h], dim=1)
        self.out = self.output(h)

        return self.out


def resnet18_fpn(in_channels, out_channels, pretrained=False):
    base = resnet18(in_channels, pretrained)
...
def resnet34_fpn(in_channels, out_channels, pretrained=False):
    base = resnet34(in_channels, pretrained)
    model = ResNet_fpn(base, in_channels, 512, out_channels)
    if pretrained:
        pretrained_dict = base.state_dict()
        model_dict = model.state_dict()
...
def resnet50_fpn(in_channels, out_channels, pretrained=False):
    base = resnet50(in_channels, pretrained)
...
def resnet101_fpn(in_channels, out_channels, pretrained=False):
    base = resnet101(in_channels, pretrained)
    model = ResNet_fpn(base, in_channels, 2048, out_channels)
    if pretrained:
        pretrained_dict = base.state_dict()
        model_dict = model.state_dict()
...
def resnet152_fpn(in_channels, out_channels, pretrained=False):
    base = resnet152(in_channels, pretrained)
    model = ResNet_fpn(base, in_channels, 2048, out_channels)
    if pretrained:
        pretrained_dict = base.state_dict()
        model_dict = model.state_dict()
