...
class ResNet(nn.Module):
    ...
    def __init__(self, block, in_channels, stage_layernum, num_classes=1000):
        ...
        self.layer1 = self._make_stage(block, 64, 64, stage_layernum[0], first_stride=1)
        self.layer2 = self._make_stage(block, 64 * block.expansion, 128, stage_layernum[1], first_stride=2)
        self.layer3 = self._make_stage(block, 128 * block.expansion, 256, stage_layernum[2], first_stride=2)
        self.layer4 = self._make_stage(block, 256 * block.expansion, 512, stage_layernum[3], first_stride=2)
    ...
    def _make_stage(self, block, in_channels, channels, layernum, first_stride):
        downsample = None
        if first_stride > 1 or in_channels != channels * block.expansion:
            downsample = nn.Sequential(
                nn.Conv2d(in_channels, channels * block.expansion, 1, stride=first_stride, bias=False),
                nn.BatchNorm2d(channels * block.expansion)
            )
        layers = [block(in_channels, channels, first_stride, downsample)]
